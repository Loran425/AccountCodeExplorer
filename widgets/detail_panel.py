...
class DetailView(ttk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        for i, field in enumerate(AccountCodeFields):
            ...
            value = tk.Text(self, state="disabled", height=1, width=60)
        ...
        for i, flag in enumerate(AccountCodeFlags):
            ...
            label = ttk.Label(self, text=f"{flag_label_text}:")
            label.grid(row=i, column=2, sticky="w")
            var = BooleanVar(value=False)
            checkbox = ttk.Checkbutton(self, variable=var, state="disabled")
            checkbox.grid(row=i, column=3, sticky="w")
            self.detail_widgets[flag] = var

        # Add a read-only multi-line text box for description
        description_label = ttk.Label(self, text="Description:", anchor="e")
        description_label.grid(row=1, column=0, sticky="ne")
        self.description_text = tk.Text(self, height=4.75, wrap="word", state="disabled", width=60)
        ...
        notes_label = ttk.Label(self, text="District Notes:")
        notes_label.grid(row=len(AccountCodeFlags) - 1, column=0, columnspan=4, sticky="w")
        self.notes_text = tk.Text(self, height=10, wrap="word", state="disabled")
        ...
        pnotes_label = ttk.Label(self, text="Personal Notes:")
        pnotes_label.grid(row=len(AccountCodeFlags) + 10, column=0, columnspan=4, sticky="w")
        self.pnotes_text = tk.Text(self, height=10, wrap="word")
    ...
    def update_details(self, acct_code: AccountCode):
